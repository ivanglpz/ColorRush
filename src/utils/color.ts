...
const getArgs = ({
  ...
  group = 20,
  sample = 10,
}: Args): Args => ({ amount, format, group, sample });
...
const format = (input: Input, args: Args): Output => {
  const list = input.map((val) => {
    ...
    return args.format === "hex" ? rgbToHex(rgb) : "";
  });
...
const process = (handler: Handler, item: Item, args: Args): Promise<Output> =>
  new Promise((resolve, reject) =>
...
const average = (item: Item, args: Args) => process(getAverage, item, args);
const prominent = (item: Item, args: Args) => process(getProminent, item, args);
